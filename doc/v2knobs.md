...
To set `KNOB_MODE` low, `OUT` must be set low whenever row R2 is activated in the key matrix scanning loop. However, you cannot set `OUT` low continuously, because that would disable the OLED when rows R3 or R4 are selected. Refer to the [page on handshaking](handshaking.md) for more information about control signal multiplexing.
